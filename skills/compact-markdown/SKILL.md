...
- **7. Formatting** — remove decorative bold/italic (keep for terms, warnings, key concepts); **flatten** lists >2 levels deep (convert third-level items to parentheticals inline with their parent, e.g., `- OAuth 2.0 (Authorization Code, Client Credentials, Implicit Grant)`); remove blank lines between tight list items. Always use the word "flatten" when describing this transformation.
