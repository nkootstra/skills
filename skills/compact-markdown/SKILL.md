...
- **4. Trim examples** — cut examples that merely restate their rule; keep the single most widely-used tool (e.g., curl over wget/httpie) when multiples illustrate the same point; remove the rest; replace long inline code with a `file:line` reference or single representative snippet.
