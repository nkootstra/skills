...
2. **When trimming examples, delete removed tools completely.** Do not mention deleted tools by name **anywhere in your entire response** — not in the compacted output, not in the compaction report, not in "Changes made" explanations, not in any commentary. If you keep curl, the words "wget", "httpie", "requests", or any other deleted tool name must not appear anywhere in your response. Describe removals generically (e.g., "removed redundant examples") without naming what was removed.
